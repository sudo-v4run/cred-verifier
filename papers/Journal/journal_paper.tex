...
\bottomrule
\multicolumn{3}{@{}l}{\scriptsize $^\ddagger$ Prototype implementation; SHA-256 required for production (see Section~\ref{sec:discussion}).}\\
\end{tabular}
\end{table}

The \texttt{certificate\_hash} is produced by \texttt{computeCertificateHash()} in \texttt{CertificateManager.mo}. In the current prototype the seven core fields are concatenated, UTF-8 encoded, and hex-encoded:
\begin{multline}
  H_{\text{proto}} = \texttt{bytesToHex}\bigl(\text{UTF-8}(\\
    \texttt{id}\,\|\,\texttt{issuer}\,\|\,\texttt{name}\,\|\,\texttt{student\_id}\,\|\,\texttt{degree}\,\|\,\texttt{major}\,\|\,\texttt{date})\bigr)
  \label{eq:cert-hash}
\end{multline}
In production this function should be replaced with a collision-resistant primitive such as SHA-256 (see Section~\ref{sec:discussion}). This hash becomes a leaf in the Merkle tree.
