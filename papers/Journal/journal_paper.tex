...
\textit{Note}: in the current prototype, \texttt{registerUniversity} is a public call that any ICP principal can invoke without restriction. For this security property to hold in production, that function must be gated to the canister controller or an NNS-governed whitelist.
...
The \texttt{registerUniversity()} function is currently a public, permissionless call: any ICP principal can invoke it and immediately gain the ability to issue credentials. This is acceptable for a research prototype but means the ``Unauthorized Issuance'' security guarantee does not hold against a motivated attacker on the live canister. Production hardening requires restricting this call to the canister controller, an admin principal, or an NNS-governed governance proposal.
