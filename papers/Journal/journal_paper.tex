...
\texttt{certificate\_hash}  & Text   & Hex-encoded UTF-8 of concatenated fields (prototype; see Section~\ref{sec:discussion}) \\
...
\begin{equation}
  H_{\text{proto}} = \texttt{bytesToHex}\bigl(\text{UTF-8}(\texttt{id}\,\|\,\texttt{issuer}\,\|\,\texttt{name}\,\|\,\texttt{student\_id}\,\|\,\texttt{degree}\,\|\,\texttt{major}\,\|\,\texttt{date})\bigr)
  \label{eq:cert-hash}
\end{equation}
...
\begin{equation}
  \texttt{nodes}[j] = \texttt{combineHashes}(\texttt{nodes}[2j+1],\; \texttt{nodes}[2j+2])
  \label{eq:merkle}
\end{equation}
