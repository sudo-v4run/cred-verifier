...
The best estimate is $\approx$\textbf{\$0.00002 per certificate} (8--17\,M cycles). The previously stated \$0.001 would require $\approx$738\,M Wasm instructions, implying a 7{,}000$\times$ overhead factor over raw instruction counts -- inconsistent with the GC-optimized implementation. Even the upper bound (\$0.000023) is $>$\textbf{8{,}000$\times$ cheaper} than Ethereum's \$0.20--\$2.00 per credential write. Direct cycle instrumentation via ICP's \texttt{ExperimentalInternetComputer.countInstructions} API~\cite{dfinity2025gascost} is left to future work.
