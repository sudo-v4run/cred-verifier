...
Computation on ICP is paid for using \emph{cycles}, which are pegged to the Swiss Franc at a fixed exchange rate (1 trillion cycles = 1 XDR $\approx$ \$1.35). Canister operators ``top up'' their canisters with cycles, and each operation (update call, memory storage, network bandwidth) consumes a known amount. Critically, \emph{query calls are free} for the caller — the canister operator absorbs the cost. This means that credential verification in TrustVault costs nothing to the employer or verifier, regardless of how many queries they make. Issuance (an update call) costs roughly \$0.001 per certificate at current ICP cycle prices (approximately 1~trillion cycles $\approx$ \$1.35 USD), making credential issuance orders of magnitude cheaper than Ethereum.
...
Issuance cost         & \textbf{\$0.001} (est.$^\dagger$)   & \$0.20--\$2.00~\cite{wood2014ethereum} & Near-zero \\
...
\begin{itemize}[leftmargin=*]
  ...
  \item ICP's per-credential cost (\$0.001) is 200--2{,}000$\times$ cheaper than Ethereum. Note: the Ethereum cost range~\cite{wood2014ethereum} reflects pre-EIP-1559 baselines; post-Merge costs vary widely with network demand and have exceeded \$50 per transaction in high-demand periods.
...
The \$0.001 issuance cost reported throughout this paper is an estimate derived from ICP's public cycle pricing documentation (1\,trillion cycles $\approx$ 1\,XDR $\approx$ \$1.35\,USD); it is not directly instrumented in the benchmark suite, which measures only timing. The full \texttt{issueCertificate} execution consumes more cycles than just the base ingress fee: it includes HashMap allocation and insertion, certificate record creation, Merkle tree path recomputation (O($\log n$) hash calls), CertifiedData.set(), and PerformanceMetrics recording. The total for a representative call is estimated at $\sim$700\,million cycles $\approx$ \$0.001, making it orders of magnitude cheaper than Ethereum gas costs. Direct cycle instrumentation via the ICP management canister's \texttt{canister\_status} API is left to future work.
