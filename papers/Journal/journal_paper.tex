...
This paper presents \textbf{TrustVault}, an academic credential verification system that is \emph{fully on-chain} — both the data and the user-facing verification interface reside on the Internet Computer Protocol (ICP) blockchain. TrustVault uses an incremental Merkle tree combined with ICP's \emph{Certified Variables} (values signed by the subnet's threshold BLS key) so that anyone can verify a credential mathematically, without trusting any single server or institution. We implemented and evaluated TrustVault on the ICP mainnet, running a comprehensive benchmark suite in March 2026. Our evaluation showed: single-certificate issuance in about 1.9 seconds, verification queries as fast as 60 milliseconds, parallel throughput up to 7.3 certificates per second, concurrent mixed-workload throughput of 13.1 operations per second with zero errors, block finality of 2.40 seconds (p99 under 3.05 seconds), and an issuance cost of approximately \$0.00002 per certificate — orders of magnitude cheaper than Ethereum.
\end{abstract}
...
Academic credential fraud is a serious and growing problem. Studies estimate that roughly one in five job applications in certain regions contain misrepresented qualifications~\cite{chahal2021credential}. When an employer wants to check whether a candidate truly holds a claimed degree, the typical process involves contacting the issuing university, waiting days for a response, and trusting that the reply is genuine. This manual approach is slow, expensive, and does not scale to global hiring.
...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{Public blockchains like Ethereum}~\cite{buterin2014ethereum} offer strong tamper resistance, but writing a single credential record costs \$0.20--\$2.00 in gas fees depending on network congestion~\cite{wood2014ethereum}, making it impractical for universities that issue thousands of degrees annually. Moreover, the verification interface (website) still runs on a regular web server.
  ...
  \item \textbf{Hybrid systems} that store only hashes on-chain while keeping actual data on a separate server reintroduce centralization for the off-chain component.
\end{itemize}
...
\begin{enumerate}
  \item We design and implement \textbf{TrustVault}, among the first fully on-chain academic credential verification systems — with both frontend and backend on a public blockchain mainnet — and provide comprehensive reproducible mainnet benchmarks that demonstrate practical viability.
...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{Permissioned blockchains} like Hyperledger Fabric~\cite{androulaki2018fabric} restrict participation to known entities. They achieve higher throughput (up to 3,500 transactions per second~\cite{sharmaperformance2019}) and lower cost, but the verifier must trust the consortium that operates the chain.
\end{itemize}

\subsection{Internet Computer Protocol (ICP)}
\label{subsec:icp}

The Internet Computer Protocol~\cite{hanke2018dfinity,dfinity2022geeks} is a public blockchain that takes a fundamentally different approach. Instead of running a single global chain, ICP operates multiple independent \emph{subnets}, each consisting of 13--40 geographically distributed \emph{replica nodes} operated by independent entities. Each subnet runs its own consensus protocol and hosts a set of smart contracts called \emph{canisters}.
...
Computation on ICP is paid for using \emph{cycles}, which are pegged to the Swiss Franc at a fixed exchange rate (1 trillion cycles = 1 XDR $\approx$ \$1.35). Canister operators ``top up'' their canisters with cycles, and each operation (update call, memory storage, network bandwidth) consumes a known amount. Critically, \emph{query calls are free} for the caller — the canister operator absorbs the cost. This means that credential verification in TrustVault costs nothing to the employer or verifier, regardless of how many queries they make. Issuance (an update call) costs approximately \$0.00002 per certificate~\cite{dfinity2025gascost} (estimated from ICP's official pricing formula; see Section~\ref{sec:discussion} for the full derivation), making credential issuance orders of magnitude cheaper than Ethereum.
...
TrustVault consists of two ICP canisters implemented and evaluated on the public ICP mainnet: a \emph{backend canister} written in Motoko (ICP's native language) that stores credentials and handles business logic, and a \emph{frontend canister} (an asset canister) that hosts a React single-page application served directly over HTTPS from the blockchain.
...
\begin{tikzpicture}[
  ...
  \node[actor] (uni) at (0.0, 4.05) {University\\(Issuer)};
  \node[actor] (stu) at (2.0, 4.05) {Student\\(Holder)};
  \node[actor] (ver) at (4.0, 4.05) {Verifier\\(Employer)};
  \node[font=\scriptsize\bfseries, icpblue] at (2.0, 4.55) {--- ICP Mainnet ---};
  \node[certd] (cd)  at (2.0, -1.10) {Certified Data API};
  ...
  \draw[arr, dashed, gray] (stu.south) -- node[right=1pt, font=\tiny, pos=0.5]{HTTPS} (front.north);
  \draw[arr, dashed, gray] (ver.south) -- node[right=2pt, font=\tiny, pos=0.5]{HTTPS} (front.north east);
  \draw[arr] (front.south) -- node[right=2pt, font=\tiny]{Candid} (main.north);
...
\begin{enumerate}
  ...
  \item \textbf{Utils.mo} — Provides hex encoding and hash-combination utilities (XOR-based in the prototype; SHA-256 in production).
...
\texttt{certificate\_hash}  & Text   & UTF-8 hex of core fields$^\ddagger$ \\
\texttt{issuer}             & Record & Institution name, ID, URL \\
\texttt{recipient}          & Record & Student name, ID, principal \\
\texttt{credential}         & Record & Degree, major, GPA, dates \\
...
\bottomrule
\multicolumn{3}{@{}p{\columnwidth}@{}}{\scriptsize $^\ddagger$ Prototype; SHA-256 required for production (see Section~\ref{sec:discussion}).}\\
\end{tabularx}
\end{table}

The \texttt{certificate\_hash} is produced by \texttt{computeCertificateHash()} (\texttt{CertificateManager.mo}). In the prototype, seven core fields (\texttt{id}, \texttt{issuer}, \texttt{name}, \texttt{student\_id}, \texttt{degree}, \texttt{major}, \texttt{date}) are concatenated, UTF-8 encoded, and hex-encoded. Denoting their concatenation as $\mathbf{f}_{1..7}$:
\begin{equation}
  H_{\text{proto}} = \texttt{bytesToHex}\bigl(\text{UTF-8}(\mathbf{f}_{1..7})\bigr)
  \label{eq:cert-hash}
\end{equation}
In production this should be replaced with SHA-256 (see Section~\ref{sec:discussion}). This hash becomes a leaf in the Merkle tree.
...
\begin{enumerate}
  ...
  \item \textbf{University issues credential}: The university fills in the student's information in the University Portal and clicks ``Issue.'' The backend creates the certificate record, computes its certificate hash (\texttt{computeCertificateHash()}), inserts the hash into the Merkle tree, and commits the new root via \texttt{CertifiedData.set()}.
...
\begin{multline}
  \texttt{nodes}[j] = \\
    \texttt{combineHashes}(\texttt{nodes}[2j{+}1],\; \texttt{nodes}[2j{+}2])
  \label{eq:merkle}
\end{multline}
In the prototype \texttt{combineHashes()} XORs the character bytes of its two hex-string inputs into a 32-byte array and hex-encodes the result. In production this should be replaced by $\text{SHA-256}(\texttt{nodes}[2j+1] \| \texttt{nodes}[2j+2])$ (see Section~\ref{sec:discussion}).
...
\textbf{Proof generation} (O($n$) in current implementation): To prove that a certificate exists in the tree, the system linearly scans all $n$ leaves to locate the target leaf hash (O($n$)), then walks O($\log n$) nodes to the root collecting sibling hashes. An index-based lookup would reduce this to O($\log n$); that optimization is left to future work.
...
\begin{tikzpicture}[
  ...
  %% Leaf spacing 0.9; every parent centered over its children
  \node[root]  (r)   at (3.15, 0)    {$R$};
  \node[inode] (n1)  at (1.35,-0.9)  {$h_{01}$};
  \node[inode] (n2)  at (4.95,-0.9)  {$h_{23}$};
  \node[inode] (n3)  at (0.45, -1.9) {$h_0$};
  \node[inode] (n4)  at (2.25, -1.9) {$h_1$};
  \node[inode] (n5)  at (4.05, -1.9) {$h_2$};
  \node[inode] (n6)  at (5.85, -1.9) {$h_3$};
  \node[leaf]  (l0)  at (0.0,  -2.9) {$c_0$};
  \node[leaf]  (l1)  at (0.9,  -2.9) {$c_1$};
  \node[leaf]  (l2)  at (1.8,  -2.9) {$c_2$};
  \node[leaf]  (l3)  at (2.7,  -2.9) {$c_3$};
  \node[leaf]  (l4)  at (3.6,  -2.9) {$c_4$};
  \node[leaf]  (l5)  at (4.5,  -2.9) {$c_5$};
  \node[leaf,draw=icpred,very thick] (l6) at (5.4,-2.9) {$c_6$};
  \node[leaf,fill=lightblue!30,dashed] (l7) at (6.3,-2.9){$\varnothing$};
  ...
  \node[draw, rounded corners, fill=lightpurple!60, minimum width=2.0cm,
       ...
       at (3.15, 0.85) {Subnet BLS signs $R$};
...
\begin{enumerate}
  \item \textbf{Hash check}: Recompute $h_c = H(\text{certificate fields})$ and confirm it matches the stored \texttt{certificate\_hash} ($H$ is SHA-256 in production; bytesToHex(UTF-8(\textbf{f}$_{1..7}$)) in the current prototype — see Section~\ref{sec:discussion}).
...
The security properties of the Merkle tree and verification protocol are analyzed in detail in Section~\ref{sec:security}.

% =========================================================
\section{Security Analysis}
\label{sec:security}

TrustVault's central claim is \emph{trustless verification}: a verifier does not need to trust any single server, institution, or even the canister code to confirm that a credential is authentic. This section provides a formal threat-model-based security analysis.

\subsection{Threat Model}

We consider the following adversarial capabilities. A \emph{malicious issuer} attempts to issue forged or altered credentials. A \emph{tampered storage adversary} attempts to modify stored certificate data on the canister. A \emph{network adversary} (MITM) intercepts or modifies responses in transit between client and boundary node. A \emph{replay attacker} reuses a valid certificate or submits duplicate issuance requests. A \emph{compromised node subset} controls up to $f < n/3$ subnet replicas that behave arbitrarily. We assume: (1)~SHA-256 is collision-resistant; (2)~BLS12-381 is secure under the Computational Diffie--Hellman (CDH) assumption~\cite{boneh2001bls}; (3)~the ICP subnet is honest-majority at any given time.

We now analyze how each adversarial capability is countered by TrustVault's design.

\subsection{Credential Forgery}

\textit{Scope of this analysis:} The properties below describe the system as designed for production with SHA-256 as the hash function. The current prototype uses simplified placeholder functions (UTF-8 hex encoding for certificate hashes; XOR-based combination for Merkle internal nodes) that do not provide cryptographic collision resistance. These must be replaced with SHA-256 before the guarantees below hold; see Section~\ref{sec:discussion} for this limitation.
...
\begin{enumerate}
  \item \textbf{Find a SHA-256 collision}: Produce a fake certificate whose SHA-256 hash matches an existing leaf in the Merkle tree. SHA-256 has $2^{256}$ possible outputs; finding a collision requires approximately $2^{128}$ operations (birthday bound), which is computationally infeasible with current or foreseeable hardware.
  \item \textbf{Forge the Merkle proof}: Construct a set of sibling hashes that, together with the fake certificate's hash, produce the correct certified root. Since each level of the tree applies SHA-256, this reduces to finding a preimage for SHA-256 at every level — a chain of $\lceil\log_2 n\rceil$ preimage attacks, each requiring $2^{256}$ operations.
  \item \textbf{Forge the BLS signature}: Produce a valid BLS signature over a state tree containing a fake root, without holding the subnet's private key. Breaking BLS-381 requires solving the Computational Diffie--Hellman (CDH) problem on a pairing-friendly curve, which is computationally infeasible at the 128-bit security level~\cite{boneh2001bls}.
\end{enumerate}
...
\subsection{Credential Tampering}

An attacker who modifies even a single byte of a stored certificate (e.g., changing a grade or graduation date) changes its SHA-256 hash. The Merkle proof computed from the original hash no longer matches the modified certificate. The verifier detects this mismatch in Step~1 of the verification protocol (Section~\ref{sec:merkle}): recomputing $h_c$ from the certificate fields produces a different hash than the stored \texttt{certificate\_hash}. No network call is needed — the check is purely local.

\subsection{Unauthorized Issuance}

The \texttt{issueCertificate} function checks that the caller's ICP Principal is in the registered university registry. An attacker who is not a registered university receives an ``Unauthorized'' error before any state change occurs.

To bypass this, the attacker would need to either:
\begin{itemize}[leftmargin=*]
  \item Steal a registered university's Ed25519 private key (requires compromising the university's key management).
  \item Compromise the canister code to remove the access check (requires controlling the canister's controller Principal, which in a production deployment can be set to the NNS DAO or blackholed entirely).
\end{itemize}

Neither is a vulnerability in TrustVault's design — they are operational security requirements that apply to any system.

\textit{Note}: for ease of prototype testing, \texttt{registerUniversity} is a permissionless call that any ICP principal can invoke without restriction. This is an intentional development convenience. A production deployment would restrict this to the canister controller or an NNS-governed whitelist with appropriate identity validation.
...
\begin{itemize}[leftmargin=*]
  \item \textbf{Credential data}: The Merkle proof and BLS signature are verified client-side. Even if a MITM modifies the response, the mathematical verification fails because the modified data does not match the BLS-signed root.
  \item \textbf{Frontend code}: ICP's HTTP gateway includes \emph{certified HTTP headers} — cryptographic signatures over the response body. A MITM who injects malicious JavaScript into the frontend would produce a response whose body does not match the certified header signature, and the tampering is detectable~\cite{dfinity2023httpgateway}.
\end{itemize}

\subsection{Subnet Compromise}
...
\subsection{Summary of Trust Assumptions}

TrustVault's trustless verification rests on the following assumptions:
\begin{enumerate}
  \item \textbf{SHA-256 is collision-resistant}: No one can find two inputs with the same SHA-256 output. This is a standard assumption in all modern cryptographic systems.
  \item \textbf{BLS12-381 is secure}: The CDH problem on the BLS12-381 pairing-friendly curve is computationally infeasible~\cite{boneh2001bls}, ensuring threshold BLS signatures cannot be forged without controlling the full signing key.
  \item \textbf{The ICP subnet is honest-majority}: Fewer than one-third of the subnet's nodes are malicious at any given time. This is enforced by ICP's decentralized node selection and NNS governance.
  \item \textbf{NNS governance integrity}: The Network Nervous System, through which subnet keys are managed and protocol upgrades are deployed, is not compromised. All upgrades require community approval via NNS voting — DFINITY cannot unilaterally change the platform — but this remains a trust assumption not present in a fully permissionless system.
  \item \textbf{Collision-resistant hashing}: The certificate hash function and Merkle tree node combination use SHA-256 or an equivalent collision-resistant primitive. The current prototype substitutes simplified placeholder functions; this assumption is not satisfied until they are replaced.
\end{enumerate}
...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{Deployment}: Both canisters were deployed on ICP mainnet using the \texttt{dfx} CLI tool provided by DFINITY. The backend canister ID is \texttt{pekzw-diaaa-aaaad-afh3q-cai} (active during benchmarking, March 2026).
\end{itemize}
...
Listing~\ref{lst:issue} shows the simplified issuance flow. The three key steps are: (1) verify that the caller is a registered university, (2) create the certificate record and compute its certificate hash (\texttt{computeCertificateHash()}), and (3) insert the hash as a Merkle leaf and commit the new root via \texttt{CertifiedData.set()}.
...
public func insertLeaf(
    ...
    while (cur > 0) {
        let p = (cur - 1) / 2;
        state.nodes[p] := combineHashes(
            state.nodes[2*p+1], state.nodes[2*p+2]);
        // prototype: XOR-based; production: SHA-256
        cur := p;
    };
...
All benchmarks were run against the live ICP mainnet canister \texttt{pekzw-diaaa-aaaad-afh3q-cai} in March 2026 using a Node.js benchmark suite (\texttt{@dfinity/agent}) from Bengaluru, India (approximately 50--80~ms round-trip time to ICP boundary nodes). Each experiment was repeated 3 times; we acknowledge that confidence intervals are wider with three trials than with the 10--30 trials common in high-throughput benchmarking studies~\cite{dinh2017blockbench}. We report mean, standard deviation, and percentiles (p50, p95, p99). These measurements reflect network conditions from Bengaluru; users closer to ICP boundary nodes would observe lower absolute latencies.
...
\begin{tabular}{@{}crrrr@{}}
\toprule
\textbf{Batch ($n$)} & \textbf{Wall (ms)} & \textbf{p95 (ms)} & \textbf{Thpt (certs/s)} & \textbf{Std.\ Dev} \\
\midrule
...
\begin{axis}[
  ...
  \addplot[fill=icpblue!70, draw=icpblue]
    coordinates {(n=1,202)(n=10,345)(n=100,400)(n=500,381)};
  \addlegendentry{p50}
  \addplot[fill=icpyellow!80, draw=icpyellow!60!black]
    coordinates {(n=1,396)(n=10,600)(n=100,693)(n=500,644)};
  \addlegendentry{p95}
  \addplot[fill=icpred!70, draw=icpred]
    coordinates {(n=1,414)(n=10,627)(n=100,937)(n=500,829)};
  \addlegendentry{p99}
\end{axis}
\end{tikzpicture}
\caption{Verification latency (p50/p95/p99) for different batch sizes. Each batch of $n$ queries is issued sequentially. p99 stays below 1 second across all scales.}
...
\end{axis}
\end{tikzpicture}
\caption{Latency by operation type. Verification stays under 640~ms even at $c = 50$. Issuance grows gradually due to consensus serialization.}
\label{fig:conc_lat}
\end{figure}
...
The mean finality of \textbf{2.40 seconds} with a tight p99 of \textbf{3.05 seconds} confirms that ICP's consensus is fast and predictable. This makes TrustVault suitable for interactive use cases — an employer can verify a credential in real time during a job interview.
...
\midrule
Block finality        & \textbf{2.40~s} (measured)    & $\approx$24~s~\cite{buterin2020eth2}  & $<$1~s~\cite{androulaki2018fabric} \\
...
\midrule
\multicolumn{4}{@{}l}{\scriptsize $^\dagger$\,Estimated from code-complexity analysis against ICP\'s official pricing formula~\cite{dfinity2025gascost}; see Section~\ref{sec:discussion} for full derivation.} \\
\bottomrule
\end{tabular}
\end{table*}
...
Key observations:
\begin{itemize}[leftmargin=*]
  \item ICP's finality (2.40~s) is about 10$\times$ faster than Ethereum PoS (24~s) but slower than Fabric ($<$1~s). However, Fabric requires trusting the consortium.
  \item ICP's per-credential cost ($\approx$\$0.00002) is ${>}$10{,}000$\times$ cheaper than Ethereum. Note: the Ethereum cost range~\cite{wood2014ethereum} reflects pre-EIP-1559 baselines; post-Merge costs vary widely with network demand and have exceeded \$50 per transaction in high-demand periods.
  ...
  \item Fabric achieves far higher raw throughput (3{,}500 TPS), but this is for a permissioned network where all validators are known and trusted — a fundamentally different security model. The Fabric figure is also a general-transaction benchmark~\cite{androulaki2018fabric}, not a credential-issuance measurement; a matched-workload comparison would require running Fabric with equivalent credential schemas.
\end{itemize}
...
The benchmark results demonstrate that TrustVault is practical for real-world use. Consider a mid-sized university that graduates 5,000 students per year. At 7.3 certs/s parallel throughput, all credentials could be issued in approximately 11 minutes. At $\approx$\$0.00002 per credential~\cite{dfinity2025gascost}, the total annual issuance cost would be about \$0.10 — essentially free compared to the cost of maintaining a traditional verification office.
...
\subsubsection{Throughput Ceiling}
...
\subsubsection{Issuance Cost Estimation}
A thorough code-complexity analysis against ICP's official pricing formula~\cite{dfinity2025gascost} (updated Dec.\ 2025) gives the following per-call breakdown. On a 13-node subnet, ICP charges: (i)~1{,}200{,}000 cycles (base) + 2{,}000 cycles per byte for ingress messages; (ii)~5{,}000{,}000 cycles per update message execution; and (iii)~1 cycle per executed WebAssembly (Wasm) instruction~\cite{dfinity2025gascost}. At 1\,trillion cycles $= 1$\,XDR $\approx$ \$1.354820\,USD (May 2025), 1\,billion cycles $\approx$ \$0.001355.

\paragraph{Payload.}
\texttt{issueCertificate} takes 11 text fields and one float (\texttt{gpa}). A representative call (names~$\approx$30\,B, IDs~$\approx$15\,B, dates~$\approx$10\,B, degree/major/URL~$\approx$30\,B each, Candid framing~$\approx$50\,B) totals $\approx$400\,bytes.

\paragraph{Wasm instruction count.}
We trace each call site through the Motoko source:
\begin{itemize}[leftmargin=*]
  \item \textbf{Access control and duplicate check} (2$\times$ \texttt{HashMap.get}): $\approx$800 raw Wasm instructions.
  \item \textbf{\texttt{createCertificate}}: 6 string concatenations ($\approx$150\,chars total), \texttt{Text.encodeUtf8}, \texttt{Blob.toArray}, \texttt{bytesToHex} (\texttt{Array.tabulate} + \texttt{Text.fromIter} over 300-char output): $\approx$33{,}000 raw instructions.
  \item \textbf{\texttt{HashMap.put}}: $\approx$1{,}500 raw instructions.
  \item \textbf{\texttt{insertCertLeaf}} with O($\log n$) \texttt{combineHashes} (7 levels for $n{=}100$): Level~0 combines two $\approx$300-char leaf hashes; Levels~1--6 combine 64-char internal nodes; each level iterates char-by-char + \texttt{bytesToHex}(32\,B): $\approx$56{,}000 raw instructions total.
  \item \textbf{\texttt{CertifiedData.set} + \texttt{textTo32ByteBlob}}: $\approx$3{,}200 raw instructions.
  \item \textbf{\texttt{recordMetric}$\times$2 + miscellaneous}: $\approx$3{,}300 raw instructions.
\end{itemize}

Total raw Wasm instructions: $\approx$100{,}000. The Motoko runtime adds GC tracing, type-tag checks, and heap bounds-checking overhead. The current code is already GC-optimized (\texttt{bytesToHex} uses \texttt{Array.tabulate} instead of $O(N^2)$ string accumulation; \texttt{combineHashes} avoids intermediate string allocations). Applying a conservative 10--100$\times$ runtime overhead factor yields $\approx$1--10\,M executed Wasm instructions.

\begin{table*}[t]
\caption{Estimated Cycle Cost per \texttt{issueCertificate} Call (13-node subnet, 1\,T cycles $\approx$ \$1.355)~\cite{dfinity2025gascost}}
\label{tab:cost_breakdown}
\centering\small\renewcommand{\arraystretch}{1.1}
\begin{tabular}{@{}lrr@{}}
\toprule
\textbf{Component} & \textbf{Cycles} & \textbf{USD (approx.)} \\
\midrule
...
\midrule
\textbf{Total (estimated)}            & \textbf{8 -- 17\,M}     & \textbf{\$0.000011 -- \$0.000023} \\
\bottomrule
\end{tabular}
\end{table*}

The best estimate is $\approx$\textbf{\$0.00002 per certificate} (8--17\,M cycles), which is $>$\textbf{8{,}000$\times$ cheaper} than Ethereum's \$0.20--\$2.00 per credential write.

\subsubsection{Hash Collision Risk}
...
\subsubsection{Prototype Hash Functions}
The current implementation uses simplified placeholder hash functions rather than SHA-256. Specifically, \texttt{computeCertificateHash()} in \texttt{CertificateManager.mo} encodes the concatenated certificate fields as their UTF-8 byte sequence in hexadecimal — not a cryptographic hash. \texttt{combineHashes()} in \texttt{Utils.mo} XORs the character bytes of two hex strings into a 32-byte array rather than applying SHA-256. While sufficient to produce unique identifiers for the benchmarking prototype and for demonstrating the system architecture, these functions do \emph{not} provide the collision resistance required for the security guarantees in Section~\ref{sec:security}. Replacing both with SHA-256 (e.g., via the \texttt{mo:sha2} Motoko package) is a prerequisite for production deployment.

\subsubsection{Open University Registration}
In the current prototype, \texttt{registerUniversity()} is a public, permissionless call: any ICP principal can invoke it and immediately gain the ability to issue credentials. This is an intentional convenience for development and benchmarking — it allows easy testing without an admin setup step. It does not reflect the intended production design; a deployed system would restrict this call to the canister controller, an admin principal, or an NNS-governed governance proposal. No change to the Merkle tree or Certified Variables mechanism is required.
...
\begin{itemize}[leftmargin=*]
  \item \textbf{Small university} (1,000 graduates/year): All credentials could be issued in 2.3 minutes (parallel) at a cost of \$0.02.
  \item \textbf{Large university} (50,000 graduates/year): All credentials could be issued in 1.9 hours (parallel) at a cost of \$1.
  \item \textbf{National system} (1 million graduates/year): With a single canister, issuance would take about 38 hours. With 10 canisters (multi-canister sharding), this drops to under 4 hours at a cost of approximately \$20.
...
\begin{itemize}[leftmargin=*]
  ...
  \item Verification queries as fast as 60~ms (free of charge).
  \item Parallel throughput up to 7.3 certificates per second.
  \item Concurrent mixed-workload throughput of 13.1 ops/s with zero errors.
  \item Block finality of 2.40 seconds (p99 under 3.05 seconds).
  \item Issuance cost of approximately \$0.00002 per certificate (8--17\,M cycles; estimated from code-complexity analysis against ICP's official pricing~\cite{dfinity2025gascost}).
  \item O($\log n$) Merkle tree scaling confirmed with less than 10\% throughput difference between a fresh and a 3,280+ certificate database.
\end{itemize}

Compared to Ethereum PoS, TrustVault is 10$\times$ faster in finality and 10{,}000--200{,}000$\times$ cheaper per credential. Compared to Hyperledger Fabric, TrustVault provides true zero-trust verification without requiring trust in a consortium.
...
\begin{enumerate}
  ...
  \item \textbf{Direct cycle instrumentation}: Measuring actual cycle consumption via ICP's \texttt{countInstructions} API to validate and refine the cost estimate in Section~\ref{sec:discussion}.
\end{enumerate}
