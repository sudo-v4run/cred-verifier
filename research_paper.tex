...
\end{tikzpicture}
\caption{TrustVault system architecture. University and Verifier access the on-chain Frontend canister via HTTPS. The Frontend dispatches Candid RPC calls to \texttt{main.mo}, which fans out to sub-modules via a shared dispatch bus. The MerkleTree module commits the certified root via \texttt{CertifiedData.set()}, BLS-signed by the subnet each consensus round.}
\label{fig:sysarch}
\end{figure}
