...
\begin{axis}[
  ...
  \addplot[fill=icpyellow!80, draw=icpyellow!60!black]
    coordinates {(n=1,2.11)(n=10,1.86)(n=50,6.31)(n=100,14.66)};
  \addlegendentry{p95}
  \addplot[fill=icpred!70,   draw=icpred]
    coordinates {(n=1,2.13)(n=10,1.90)(n=50,6.54)(n=100,15.28)};
  \addlegendentry{p99}
\end{axis}
\end{tikzpicture}
...
\textbf{60\,ms}, approaching the raw network RTT, confirming that the
Merkle proof computation adds negligible overhead.  The p99 stays below
630\,ms even for 10-query bursts — suitable for real-time employer
...
\begin{axis}[
  ...
  \addplot[fill=icpgreen!50, draw=icpgreen!70!black]
    coordinates {(n=1,60)(n=10,60)(n=100,60)};
  \addlegendentry{Min}
  \addplot[fill=icpblue!70, draw=icpblue]
    coordinates {(n=1,202)(n=10,345)(n=100,490)};
  \addlegendentry{p50 (median)}
  \addplot[fill=icpyellow!80, draw=icpyellow!60!black]
    coordinates {(n=1,396)(n=10,600)(n=100,800)};
  \addlegendentry{p95}
  \addplot[fill=icpred!70, draw=icpred]
    coordinates {(n=1,414)(n=10,627)(n=100,900)};
  \addlegendentry{p99}
\end{axis}
\end{tikzpicture}
\caption{Verification query latency distribution (min/p50/p95/p99) for
1, 10, and 100 simultaneous queries against a 500-cert database.  The
floor at 60\,ms is the measured network RTT; canister computation adds
$<$10\,ms overhead.}
...
$\approx$3\,650 existing certs).  Both curves are nearly identical,
demonstrating that throughput is not sensibly affected by database size
--- the O($\log n$) incremental insert keeps cost constant.  Peak
throughput of \textbf{8\,certs/s} is achieved at batch size 50.
...
\begin{axis}[
  ...
  %% Merkle-growth benchmark (db $\approx$3650 certs)
  \addplot[mark=triangle*, thick, color=icpgreen, mark size=2pt]
    coordinates {(1,0.48)(10,3.0)(50,8.0)(100,6.66)};
  \addlegendentry{Merkle-growth bench (db $\approx$3\,650)}
\end{axis}
\end{tikzpicture}
...
mean latency of $1{,}820$\,ms and a sustained throughput of
...
$13{,}799$\,ms and throughput of 7.25\,certs/s, confirming that the canister handles sudden load spikes without data loss.
...
$c \in \{1, 5, 10, 25, 50\}$.  Throughput grows from 0.48\,ops/s at
$c{=}1$ to \textbf{13.09\,ops/s} at $c{=}50$, a $27{\times}$
...
latency per concurrency level.  Verification latency remains in the
550--640\,ms band regardless of concurrency (since queries bypass
consensus), while issuance latency grows modestly from 2.1\,s to
2.9\,s even at $c{=}50$ --- confirming that the consensus bottleneck
...
\begin{axis}[
  ...
  \addplot[fill=icpblue!70, draw=icpblue]
    coordinates {(c=5,2302)(c=10,2368)(c=25,2362)(c=50,2912)};
...
concurrency.  Verification latency (query, no consensus) remains flat
at $\approx$570--640\,ms; issuance grows only 27\% from $c{=}5$ to
$c{=}50$.}
\label{fig:conc_lat}
\end{figure}
...
independent update calls.  Mean finality of $2{,}400$\,ms compares
favourably against Ethereum PoS ($\approx$12\,s per slot) and is
drastically faster than Bitcoin ($\approx$600\,s).  The p99 of
$3{,}047$\,ms confirms a tight worst-case bound suitable for
...
\midrule
ms & 1443 & 2400 & 385 & 2490 & 2564 & 2987 & 3047 \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
Finality & \textbf{2.4\,s} (meas.) & $\approx$24\,s \cite{buterin2020eth2} & $<$1\,s \cite{androulaki2018fabric} \\
Network throughput & 8\,certs/s (1 canister) & 15--30\,TPS total \cite{dinh2017blockbench} & $\approx$3\,500\,TPS \cite{androulaki2018fabric} \\
...
Live mainnet benchmarks (Figs.~\ref{fig:issuance_perc}--\ref{fig:conc_lat}, Tables~\ref{tab:issuance}--\ref{tab:finality}) demonstrate that the system is practical: single-certificate issuance completes in under 2 seconds; verification queries return in 60--500\,ms; parallel issuance scales to $\approx$8\,certs/s from a single canister; concurrent mixed-load throughput reaches 13\,ops/s at $c{=}50$ with zero errors; and block finality averages 2.4\,s with p99 under 3.1\,s. Issuance costs are fractions of a US cent. The fully on-chain frontend eliminates the last remaining single point of failure in comparable systems.
