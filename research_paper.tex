...
% =========================================================
\begin{abstract}
Academic credential fraud is a growing global problem, costing institutions and employers millions of dollars annually in lost trust and fraudulent hirings. Existing solutions rely on centralized databases, trusted intermediaries, or permissioned blockchains that introduce single points of failure and require trust in at least one party. This paper presents \textit{TrustVault}, a fully on-chain academic credential verification system deployed on the Internet Computer Protocol (ICP). TrustVault stores all credential data, Merkle proofs, and the serving frontend entirely on-chain with no traditional backend servers or cloud storage. It leverages ICP's \textit{Certified Variables} mechanism — backed by threshold BLS signatures from a decentralized subnet — to enable mathematically verifiable, zero-trust credential authentication. We describe the system architecture, the incremental Merkle tree construction for O($\log n$) certificate updates, and the full cryptographic trust chain from a certificate hash up to the subnet's root key. Live mainnet benchmarks on canister \texttt{pekzw-diaaa-aaaad-afh3q-cai} show single-issuance latency of approximately 1.91\,s, verification latency as low as 60\,ms (p50 $\approx$ 432\,ms), and sustained throughput of up to 8 certificates/second under parallel load — all at a fraction of Ethereum's gas cost. Security analysis confirms resistance to data tampering, replay attacks, and unauthorized issuance. TrustVault demonstrates that ICP's unique architecture enables practical, zero-Trust academic credentialing without centralized authorities.
\end{abstract}

\begin{IEEEkeywords}
Internet Computer Protocol, Academic Credential Verification, Blockchain, Certified Variables, Merkle Tree, Zero-Trust, On-Chain Storage, Motoko, Threshold BLS Signatures
\end{IEEEkeywords}
...
Table~\ref{tab:issuance} shows issuance latency for single and batch parallel certificate issuance. Single-certificate issuance takes a mean of $1{,}943$\,ms (wall time), consistent with ICP's $\approx 2$\,s consensus round time. Because ICP processes concurrent update calls in parallel within a single consensus round, batching 10 certificates brings the individual per-certificate latency down to a mean of $1{,}391$\,ms.

\begin{table}[t]
\caption{Issuance Latency on ICP Mainnet (wall time in ms)}
\label{tab:issuance}
\centering
\renewcommand{\arraystretch}{1.15}
\begin{tabular}{@{}crrrr@{}}
\toprule
\textbf{Batch size} & \textbf{Mean (ms)} & \textbf{p50 (ms)} & \textbf{p95 (ms)} & \textbf{Thpt (cps)} \\
\midrule
1   & 1943  & 1910  & 2111  & 0.52 \\
10  & 2596  & 2794  & 3054  & 4.02 \\
50  & 6933  & 6822  & 7320  & 7.23 \\
100 & 13969 & 15222 & 15416 & 7.31 \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Verification Latency}
Verification is a \textit{query} call (no consensus), making it significantly faster. Over a database of 500 certificates on mainnet, the mean verification latency was $358$\,ms with a p50 of $345$\,ms (Table~\ref{tab:verif}). The minimum observed latency was 60\,ms — approaching pure network RTT — demonstrating that the canister computation itself adds negligible overhead.

\begin{table}[t]
\caption{Verification Latency on ICP Mainnet (database of 500 certs)}
\label{tab:verif}
\centering
\renewcommand{\arraystretch}{1.15}
\begin{tabular}{@{}crrrrr@{}}
\toprule
\textbf{Queries} & \textbf{Min} & \textbf{Mean} & \textbf{p50} & \textbf{p95} & \textbf{p99} \\
\midrule
1   & 60    & 227   & 202   & 396  & 414 \\
10  & 60    & 359   & 345   & 600  & 627 \\
100 & 60    & $\approx$490 & 490 & $\approx$800 & $\approx$900 \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Throughput Under Concurrent Load}
Fig.~\ref{fig:throughput} shows issuance throughput (certificates per second) as a function of parallel batch size. Throughput scales super-linearly from 0.52 cps at $n=1$ to a peak of $\approx8$ cps at $n=50$, after which ICP's per-round serialization cap starts to limit the gain. This demonstrates that TrustVault can handle bursts of simultaneous graduation ceremonies from multiple universities.
...
\begin{axis}[
  ...
  \addplot[mark=*, thick, color=icpblue]
    coordinates {(1,0.52)(10,4.02)(50,7.23)(100,7.31)};
  \addlegendentry{ICP Mainnet (TrustVault)}

  %% Reference line: Ethereum (hypothetical ~0.1 cps due to gas/block limit)
  \addplot[mark=square, thick, dashed, color=icpred]
    coordinates {(1,0.07)(10,0.12)(50,0.15)(100,0.16)};
  \addlegendentry{Ethereum reference\textsuperscript{†}}
\end{axis}
\end{tikzpicture}
{\scriptsize \textsuperscript{†}Ethereum reference is estimated from typical Ethereum mainnet
throughput for simple storage transactions ($\approx$15 TPS total, gas-limited).}
\caption{Issuance throughput vs.\ parallel batch size. ICP scales to $\approx8$ certs/s from a single canister; Ethereum is constrained by block gas limits.}
\label{fig:throughput}
\end{figure}

\subsection{Finality Time}
Across 20 sampled update calls, finality time (time from submission to when the ICP certified data is updated) averaged $2{,}400$\,ms with a standard deviation of 385\,ms and a p95 of $2{,}987$\,ms. This is substantially faster than Ethereum's $\approx 12$\,s per block and near-instant compared to Bitcoin's 10-minute blocks.

\subsection{Merkle Growth Cost}
As the certificate database grows, incremental O($\log n$) insertion keeps cost roughly constant. With a database of 3,650+ certificates, inserting one more certificate takes $\approx 2{,}077$\,ms (dominated by consensus latency, not computation). This confirms that TrustVault scales to institution-size deployments.
...
Live mainnet benchmarks demonstrate that the system is practical: single-certificate issuance finishes in under 2 seconds; verification queries complete in 60–500\,ms; and parallel issuance scales to approximately 8 certificates per second from a single canister. Issuance costs are fractions of a US cent — orders of magnitude cheaper than Ethereum alternatives. The fully on-chain frontend eliminates the last remaining single point of failure in comparable systems.
