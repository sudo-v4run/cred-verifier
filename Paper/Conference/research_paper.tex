...
\textbf{60\,ms}, confirming that the
Merkle proof computation adds negligible overhead.  The p99 stays below
937\,ms even for 100-query bursts — suitable for real-time employer
...
$c{=}1$ to \textbf{13.09\,ops/s} at $c{=}50$, a $\approx$27${\times}$
...
554--635\,ms range across concurrency levels (queries bypass
consensus), while issuance latency rises from 2.2\,s at $c{=}5$
to 2.9\,s at $c{=}50$ --- confirming that the consensus bottleneck
...
$\approx$554--635\,ms; issuance grows approximately 30\% from $c{=}5$ to
$c{=}50$.}
\label{fig:conc_lat}
\end{figure}
...
Live mainnet benchmarks (Figs.~\ref{fig:issuance_perc}--\ref{fig:conc_lat}, Tables~\ref{tab:issuance}--\ref{tab:finality}) demonstrate that the system is practical: single-certificate issuance completes in under 2 seconds; verification queries return in 60--937\,ms; parallel issuance scales to $\approx$7.3\,certs/s from a single canister; concurrent mixed-load throughput reaches 13.09\,ops/s at $c{=}50$ with zero errors; and block finality averages 2.40\,s with p99 under 3.05\,s. Issuance costs are fractions of a US cent. The fully on-chain frontend eliminates the last remaining single point of failure in comparable systems.
