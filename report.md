...
Academic credential fraud imposes significant costs on institutions and employers worldwide. Existing solutions rely on either centralized registries or permissioned blockchains, both of which retain trust dependencies and single points of failure. This report presents **TrustVault**, a fully on-chain academic credential verification system implemented and evaluated on the Internet Computer Protocol (ICP) mainnet. TrustVault stores both credential data and the verification frontend inside ICP canisters and leverages ICP's *Certified Variables* — backed by threshold BLS signatures from a decentralized subnet — to anchor an incremental Merkle tree root for potential zero-trust verification. Mainnet benchmarks conducted in March 2026 demonstrate sub-second verification latency, throughput exceeding 7 credentials per second, and issuance costs over 10,000× lower than comparable Ethereum operations. The system has been deployed and running continuously on ICP mainnet with no downtime. The current prototype uses simplified placeholder hash functions; production deployment requires cryptographic hash hardening with SHA-256 and client-side BLS signature verification, both of which are architecturally supported by ICP.
